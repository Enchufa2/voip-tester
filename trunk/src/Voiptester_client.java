...
public class Voiptester_client {
     ...
     *
     */
    public void registrarCliente(String hostRegistro, int puertoRegistro)
            throws Exception {

        this.controlCliente.conectarConServidor(hostRegistro, puertoRegistro);
    }
    ...
    public static void main(String args[]){
        ...
        try {
            ...
            System.setProperty("java.rmi.server.hostname", argumentosEntrada.clientHost.value);
            cliente.registrarCliente(argumentosEntrada.serverHost.value, argumentosEntrada.serverPort.value);
    ...
    public class ArgumentosEntrada{
        ...
        ArgParser parser = new ArgParser("voiptester [{-cH, --clientHost} clientHost]"
                + "[{-sH, --serverHost} serverHost] [{-sP, --serverPort} port] [{-lH, --localHost} localhost] "
                + "[{-lP, --localPort} localport] [{-rH, --rHost} remoteHost] [{-rP, --remotePort} remotePort] "
                + "[{-t, --tParameters} idt ml mpi ni] [{-e, --estimators} djplbeq] [{-dL, --downlink}]"
                + "[{-g, --gst} cN rate channels bN] [{-o, --outputDir} output_dir] [{-?, -help}]");

        StringHolder clientHost = new StringHolder("localhost");
        ...
        public ArgumentosEntrada(){
            ...
            this.parser.addOption("-cH,--clientHost %s #Set the localhost address which receives callbacks from the server (optional)", this.clientHost);
            ...
            this.parser.addOption("-dL, --downLink %v #Set the transmission direction from server to client", this.downLink);

            this.gst[0] = Integer.MAX_VALUE;
            this.gst[3] = Integer.MIN_VALUE;
            this.parser.addOption("-g, --gst %dX4 #Set to include compressed audio into messages (mandatory if PESQ is set). "
